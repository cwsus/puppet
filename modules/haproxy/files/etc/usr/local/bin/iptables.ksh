...
## PROXY SERVERS
/usr/bin/env iptables -A INPUT -i eth+ -s ${NETWORK_ADDR} -d 192.168.10.10 -m state --state NEW -m tcp -p tcp --sport 1024:65535 --dport 8080 -j ACCEPT
/usr/bin/env iptables -A OUTPUT -o eth+ -s 192.168.10.10 -d ${NETWORK_ADDR} -m state --state ESTABLISHED,RELATED -m tcp -p tcp --sport 1024:65535 --dport 8080 -j ACCEPT

WPAD SERVERS
/usr/bin/env iptables -A INPUT -i eth+ -s ${NETWORK_ADDR} -d 192.168.10.11 -m state --state NEW -m tcp -p tcp --sport 1024:65535 --dport 80 -j ACCEPT
/usr/bin/env iptables -A OUTPUT -o eth+ -s 192.168.10.11 -d ${NETWORK_ADDR} -m state --state ESTABLISHED,RELATED -m tcp -p tcp --sport 1024:65535 --dport 80 -j ACCEPT
